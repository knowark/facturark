...
from ..api import (
    build_document, send_document, verify_document,
...
class Cli:
    ...
    def parse(self, arg_list):
        ...
        build_parser.set_defaults(func=self.build_document)
        ...
        send_parser.set_defaults(func=self.send_document)
        ...
        qrcode_parser.set_defaults(func=self.qrcode)

        verify_parser = subparsers.add_parser('verify')
        verify_parser.add_argument('document_file')
        verify_parser.set_defaults(func=self.verify_document)
        ...
        query_parser.set_defaults(func=self.query_document)
    ...
    def resolve_document(self, options_dict):
        document_bytes = b''
        if options_dict.get('document_file'):
            document_bytes = self.read_file(options_dict.get('document_file'))
        return document_bytes

    def build_document(self, options_dict):
        invoice_bytes = self.read_file(options_dict.get('input_file'))
        invoice_dict = json.loads(invoice_bytes.decode('utf-8'))
        certificate = (self.read_file(options_dict.get('certificate'))
        ...
        self.write_file(output_file, invoice_xml)

    def qrcode(self, options_dict):
        document_path = options_dict.get('document_file')
        document_bytes = self.read_file(document_path)
        image_bytes = generate_qrcode(document_bytes)
        output_file = options_dict.get('output_file')
        self.write_file(output_file, image_bytes)
        return True

    def send_document(self, options_dict):
        request_bytes = self.read_file(options_dict.get('request_file'))
        request_dict = json.loads(request_bytes.decode('utf-8'))

        document_bytes = self.resolve_document(options_dict)

        request_dict['document'] = request_dict.get(
            'document') or document_bytes

        response_dict = send_document(request_dict)
        response_json = json.dumps(
            response_dict, default=json_serialize).encode('utf-8')
        output_file = options_dict.get('output_file')
        self.write_file(output_file, response_json)

    def verify_document(self, options_dict):
        document_path = options_dict.get('document_file')
        document_bytes = self.read_file(document_path)
        return verify_document(document_bytes)

    def query_document(self, options_dict):
        query_bytes = self.read_file(options_dict.get('query_file'))
