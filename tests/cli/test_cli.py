import facturark.cli.cli
from pytest import raises, fixture
from facturark.cli import Cli
...
def test_cli_build_invoice(tmpdir, monkeypatch, cli):
    ...
    monkeypatch.setattr(
        facturark.cli.cli, 'build_document', mock_build_document)
    ...
    # Call The Cli
    cli.build_document(options_dict)
...
def test_cli_build_credit_note(tmpdir, monkeypatch, cli):
    test_data = {
    ...
    def mock_build_document(credit_note_dict,
                            pkcs12_certificate=None,
                            pkcs12_password=None,
                            technical_key=None,
                            kind='credit_note'):
        ...
        return b'<CreditNote><Id>777</Id><CreditNote>', ''
    ...
    assert test_data['test_pkcs12_password'] is None
    assert test_data['test_technical_key'] is None
    assert output_pathlocal.read('rb') == (
        b'<CreditNote><Id>777</Id><CreditNote>')


def test_cli_qrcode(tmpdir, monkeypatch, cli):
    monkeypatch.setattr(
        facturark.cli.cli, 'generate_qrcode', lambda document: b'QRIMAGE')
    ...
    # Call The Cli
    result = cli.qrcode(options_dict)

    assert result is True


def test_cli_send(tmpdir, monkeypatch, cli):
    test_data = {
    ...
    monkeypatch.setattr(
        facturark.cli.cli, 'send_document', mock_send_document)
    ...
    # Call The Cli
    cli.send_document(options_dict)
...
def test_cli_verify_document(tmpdir, monkeypatch, cli):
    monkeypatch.setattr(
        facturark.cli.cli, 'verify_document', lambda document: True)
    ...
    # Call The Cli
    result = cli.verify_document(options_dict)

    assert result is True


def test_cli_query(tmpdir, monkeypatch, cli):
    test_data = {
    ...
    monkeypatch.setattr(
        facturark.cli.cli, 'query_document', mock_query_document)
    ...
    # Call The Cli
    cli.query_document(options_dict)

    assert isinstance(test_data['test_query_dict'], dict)
    assert output_pathlocal.read('rb') == b'{"response": "success"}'


def test_resolve_document(monkeypatch, cli):
    data = {'called': False}
    ...
    cli.read_file = mock_read_file

    result = cli.resolve_document({'document_file': 'invoice.xml'})

    assert result == b'DATA'
    assert data['called'] is True


def test_resolve_document_empty(cli):
    result = cli.resolve_document({})
    assert result == b''
